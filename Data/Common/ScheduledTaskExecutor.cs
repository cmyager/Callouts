...
namespace Callouts.Data
{
    public sealed class ScheduledTaskExecutor : IDisposable
    {
        ...
        public ScheduledTaskExecutor(DiscordClient client, AsyncExecutionService async, ScheduledTask task)
        {
            this.client = client;
            this.async = async;
            this.Job = task;
        ...
        private void SendMessageCallback(object? _)
        {
            ...
            try
            {
                this.async.Execute(rem.Member.SendMessageAsync(rem.EventEmbed));
            }
